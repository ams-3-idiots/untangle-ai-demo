...
사용자가 적은 한 문장을 읽고, 하나의 할 일과 제목·날짜·시간·우선순위·예상 소요 시간·알림을 채웁니다.
...
- estimate: 걸릴 것으로 보이는 시간이 드러나면 짧은 한국어로 적습니다.("30분"·"1시간"·"2시간 30분") 단서가 없으면 null.
- reminder: 마감 전 알림 시점이 드러나면 반드시 다음 중 하나로만 적습니다: "10분 전"·"30분 전"·"1시간 전"·"3시간 전"·"1일 전". 단서가 없으면 null.
...
{"title": "치과 예약 전화하기", "due_date": "2026-07-03", "due_time": "15:00", "priority": "높음", "estimate": "30분", "reminder": "1시간 전", "memo": ""}
...
_ESTIMATE_KEYS = ("estimate", "duration", "eta", "estimated_time")
_REMINDER_KEYS = ("reminder", "alarm", "notify", "notification")
...
def _to_todo(data) -> Optional[Todo]:
    ...
    return Todo(
        ...
        estimate=_first_str(item, _ESTIMATE_KEYS) or None,
        reminder=_normalize_reminder(_first_str(item, _REMINDER_KEYS)),
        source="single_add",
    )


def _normalize_reminder(value: str) -> Optional[str]:
    """추출된 알림 값을 확정 패널 선택지(REMINDER_CHOICES)에 맞춘다.

    프롬프트가 정해진 문구만 내도록 지시하지만, 벗어난 값(또는 "없음")이 오면
    None 으로 떨궈 selectbox 가 '없음'으로 안전하게 시작하게 한다.
    """
    text = (value or "").strip()
    if text in REMINDER_CHOICES and text != REMINDER_NONE:
        return text
    return None
