...
> ℹ️ **현재 상태:** AI 호출 통로(`features/__init__.py` 의 `call_llm`)가 구현되어
> **브레인덤프(F2)** 가 실제 LLM 으로 동작한다. (쪼개기·단일 추가·상황 제안(F3~F5)은 아직 뼈대만 있어 순차 구현 예정)
> 키 없이도 화면(UI)은 정상적으로 뜨며, 이때 AI 기능은 "키를 설정해 달라"는 안내 메시지를 보여준다.

AI 기능을 실제로 쓰려면 OpenAI 또는 Claude API 키가 필요하다. 키는 **코드·커밋에 절대 넣지 않고**
`st.secrets`(`.streamlit/secrets.toml`)로 주입한다. (워크플로 규칙 W-5 참고)

1. 프로젝트 루트에 `.streamlit/secrets.toml` 파일을 만든다.
2. 발급받은 키를 적는다. **둘 중 하나만 있어도 동작**하며, 둘 다 있으면 Claude 를 우선 사용한다.
   ```toml
   ANTHROPIC_API_KEY = "sk-ant-..."   # Claude (기본 우선)
...
이 파일은 **개인 키이므로 절대 커밋하지 않는다.** (`.gitignore` 에 등록돼 있다.)
...
- **8501 포트가 이미 사용 중** → 다른 포트로 실행한다.
  ...
  ```
- **AI 기능이 "키를 설정해 달라"고 안내함** → `.streamlit/secrets.toml` 에 API 키가 없는 상태다. 위 'API 키 설정'을 참고해 키를 넣는다.
